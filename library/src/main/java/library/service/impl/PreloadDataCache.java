...
/**
 * 
 * ClassName: PreloadDataCache <br/>
 * Function: TODO ADD FUNCTION. <br/>
 * Reason: TODO ADD REASON(可选). <br/>
 * date: 2014年11月20日 下午6:22:39 <br/>
 * 预加载缓存数据
 * @author chenhao
 * @version @param <K>
 * @version @param <V>
...
public class PreloadDataCache<K, V> extends SimpleCache<K, V> {
    ...
    /** get data listener **/
    protected library.service.impl.PreloadDataCache.OnGetDataListener<K, V> onGetDataListener;

    /**
     * restore threads those getting data, to avoid multi threads get the data
     * for same key so that to save network traffic
     **/
    private transient java.util.Map<K, library.service.impl.PreloadDataCache.GetDataThread> gettingDataThreadMap = new java.util.HashMap<K, library.service.impl.PreloadDataCache.GetDataThread>();
    ...
    public static final int DEFAULT_THREAD_POOL_SIZE = library.util.SystemUtils.getDefaultThreadPoolSize(8);
     ...
     */
    public library.entity.CacheObject<V> get(K key, java.util.List<K> keyList) {
        if (key == null) {
            return null;
        }

        // if list is not null, preload data
        if (!library.util.ListUtils.isEmpty(keyList)) {
    ...
    public library.entity.CacheObject<V> get(K key) {
        ...
        library.entity.CacheObject<V> object = super.get(key);
        if (object == null && onGetDataListener != null) {
            library.service.impl.PreloadDataCache.GetDataThread getDataThread = gettingData(key);
     ...
     */
    library.entity.CacheObject<V> getFromCache(K key) {
     ...
     */
    library.entity.CacheObject<V> getFromCache(K key, java.util.List<K> keyList) {
        if (key == null) {
            return null;
        }
     ...
     */
    protected int preloadDataForward(K key, java.util.List<K> keyList, int cacheCount) {
        int gettingDataCount = 0;
        if (key != null && !library.util.ListUtils.isEmpty(keyList) && onGetDataListener != null) {
            int cachedCount = 0;
            boolean beginCount = false;
            for (int i = 0; i < keyList.size() && cachedCount <= cacheCount; i++) {
                K k = keyList.get(i);
                if (library.util.ObjectUtils.isEquals(k, key)) {
                    beginCount = true;
                    continue;
                }
     ...
     *         whose alreadey in cache
     */
    protected int preloadDataBackward(K key, java.util.List<K> keyList, int cacheCount) {
        int gettingDataCount = 0;
        if (key != null && !library.util.ListUtils.isEmpty(keyList) && onGetDataListener != null) {
            int cachedCount = 0;
            boolean beginCount = false;
     ...
     * @return
     */
    private synchronized library.service.impl.PreloadDataCache.GetDataThread gettingData(K key) {
        ...
        library.service.impl.PreloadDataCache.GetDataThread getDataThread = new library.service.impl.PreloadDataCache.GetDataThread(key, onGetDataListener);
    ...
    public PreloadDataCache(int maxSize, int threadPoolSize) {
        ...
        this.threadPool = java.util.concurrent.Executors.newFixedThreadPool(threadPoolSize);
    }
     ...
     */
    public library.service.impl.PreloadDataCache.OnGetDataListener<K, V> getOnGetDataListener() {
     ...
     */
    public void setOnGetDataListener(library.service.impl.PreloadDataCache.OnGetDataListener<K, V> onGetDataListener) {
    ...
    public boolean checkIsNetworkTypeAllowed() {
        if (connectivityManager == null && context != null) {
            connectivityManager = (android.net.ConnectivityManager) context.getSystemService(android.content.Context.CONNECTIVITY_SERVICE);
        }
    ...
    private int translateNetworkTypeToApiFlag(int networkType) {
        ...
        case android.net.ConnectivityManager.TYPE_MOBILE:
            return library.service.impl.PreloadDataCache.NETWORK_MOBILE;

        case android.net.ConnectivityManager.TYPE_WIFI:
            return library.service.impl.PreloadDataCache.NETWORK_WIFI;
    ...
    public static <K, V> library.service.impl.PreloadDataCache<K, V> loadCache(String filePath) {
        return (library.service.impl.PreloadDataCache<K, V>) library.util.SerializeUtils.deserialization(filePath);
    }
     ...
     *         2012-3-4
     */
    public interface OnGetDataListener<K, V> extends java.io.Serializable {
         ...
         */
        public library.entity.CacheObject<V> onGetData(K key);
    }
    ...
    private class GetDataThread implements Runnable {
        ...
        private K key;
        private library.service.impl.PreloadDataCache.OnGetDataListener<K, V> onGetDataListener;

        /** get data and cache finish lock, it will be released then **/
        public java.util.concurrent.CountDownLatch finishGetDataLock;

        /**
         * @param key
         * @param onGetDataListener
         */
        public GetDataThread(K key, library.service.impl.PreloadDataCache.OnGetDataListener<K, V> onGetDataListener) {
            this.key = key;
            this.onGetDataListener = onGetDataListener;
            finishGetDataLock = new java.util.concurrent.CountDownLatch(1);
        }

        public void run() {
            if (key != null && onGetDataListener != null) {
                library.entity.CacheObject<V> object = onGetDataListener.onGetData(key);
                if (object != null) {
                    put(key, object);
                }
            }
