...
public class LazyViewHolderCreator<T> implements library.adapter.ViewHolderCreator<T> {
    ...
    public static <ItemDataType> library.adapter.ViewHolderCreator<ItemDataType> create(final Object enclosingInstance, final Class<?> cls, final Object... args) {
        if (cls == null) {
        ...
        library.adapter.ViewHolderCreator lazyCreator = new LazyViewHolderCreator(constructor, instanceObjects);
        return lazyCreator;
    }

    @Override
    public library.adapter.ViewHolderBase<T> createViewHolder(int position) {
        Object object = null;
        try {
        ...
        if (object == null || !(object instanceof library.adapter.ViewHolderBase)) {
            throw new IllegalArgumentException("ViewHolderClass can not be initiated");
        }
        return (library.adapter.ViewHolderBase<T>) object;
    }
}
