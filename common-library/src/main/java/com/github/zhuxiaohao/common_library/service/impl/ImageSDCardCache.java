...
import com.github.zhuxiaohao.common_library.dao.impl.ImageSDCardCacheDaoImpl;
...
import com.github.zhuxiaohao.common_library.service.FileNameRule;
import com.github.zhuxiaohao.common_library.util.FileUtils;
...
public class ImageSDCardCache extends PreloadDataCache<String, String> {
    ...
    private ImageSDCardCache.OnImageSDCallbackListener onImageSDCallbackListener;
    /**
     ...
     * pool
     **/
    private transient ExecutorService threadPool = java.util.concurrent.Executors.newFixedThreadPool(SystemUtils.DEFAULT_THREAD_POOL_SIZE);
    /**
     ...
     **/
    private transient Map<String, java.util.HashSet<View>> viewSetMap;
    ...
    public boolean get(final String imageUrl, final List<String> urlList, final View view) {
        ...
        if (isOpenWaitingQueue) {
            synchronized (viewSetMap) {
                java.util.HashSet<View> viewSet = viewSetMap.get(imageUrl);
                if (viewSet == null) {
                    viewSet = new java.util.HashSet<View>();
    ...
    public ImageSDCardCache(int maxSize, int threadPoolSize) {
        ...
        this.viewMap = new java.util.concurrent.ConcurrentHashMap<String, View>();
        this.viewSetMap = new java.util.HashMap<String, java.util.HashSet<View>>();
        this.handler = new ImageSDCardCache.MyHandler();
    ...
    private class MyHandler extends android.os.Handler {
        ...
        public void handleMessage(android.os.Message message) {
            ...
            case WHAT_GET_IMAGE_FAILED:
                ImageSDCardCache.MessageObject object = (ImageSDCardCache.MessageObject) message.obj;
                if (object == null) {
                    break;
                }
    ...
    private void onGetSuccess(String imageUrl, String imagePath, View view, boolean isInCache) {
        ...
        try {
            ...
            onImageSDCallbackListener.onGetFailed(imageUrl, imagePath, view, new FailedReason(FailedReason.FailedType.ERROR_OUT_OF_MEMORY, e));
        }
    }
            ...
            public void run() {
                try {
                    ...
                    if (StringUtils.isEmpty(imagePath) || !FileUtils.isFileExist(imagePath)) {
                        ...
                        handler.sendMessage(handler.obtainMessage(WHAT_GET_IMAGE_FAILED, new ImageSDCardCache.MessageObject(imageUrl, imagePath, failedReason)));
                    } else {
                        handler.sendMessage(handler.obtainMessage(WHAT_GET_IMAGE_SUCCESS, new ImageSDCardCache.MessageObject(imageUrl, imagePath)));
                    }
                } catch (OutOfMemoryError e) {
                    ImageSDCardCache.MessageObject msg = new ImageSDCardCache.MessageObject(imageUrl, null, new FailedReason(FailedReason.FailedType.ERROR_OUT_OF_MEMORY, e));
