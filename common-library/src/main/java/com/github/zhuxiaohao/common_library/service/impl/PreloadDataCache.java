...
import com.github.zhuxiaohao.common_library.util.ListUtils;
...
public class PreloadDataCache<K, V> extends SimpleCache<K, V> {
    ...
    /** get data listener **/
    protected PreloadDataCache.OnGetDataListener<K, V> onGetDataListener;

    /**
     * restore threads those getting data, to avoid multi threads get the data
     * for same key so that to save network traffic
     **/
    private transient java.util.Map<K, PreloadDataCache.GetDataThread> gettingDataThreadMap = new java.util.HashMap<K, PreloadDataCache.GetDataThread>();
     ...
     */
    public CacheObject<V> get(K key, java.util.List<K> keyList) {
        if (key == null) {
            return null;
        }
    ...
    public CacheObject<V> get(K key) {
        ...
        if (object == null && onGetDataListener != null) {
            PreloadDataCache.GetDataThread getDataThread = gettingData(key);
     ...
     */
   CacheObject<V> getFromCache(K key, java.util.List<K> keyList) {
        if (key == null) {
            return null;
        }
     ...
     * @return
     */
    private synchronized PreloadDataCache.GetDataThread gettingData(K key) {
        ...
        PreloadDataCache.GetDataThread getDataThread = new PreloadDataCache.GetDataThread(key, onGetDataListener);
     ...
     */
    public PreloadDataCache.OnGetDataListener<K, V> getOnGetDataListener() {
    ...
    private class GetDataThread implements Runnable {
        ...
        private K key;
        private PreloadDataCache.OnGetDataListener<K, V> onGetDataListener;
         ...
         * @param onGetDataListener
         */
        public GetDataThread(K key, PreloadDataCache.OnGetDataListener<K, V> onGetDataListener) {
            this.key = key;
            this.onGetDataListener = onGetDataListener;
