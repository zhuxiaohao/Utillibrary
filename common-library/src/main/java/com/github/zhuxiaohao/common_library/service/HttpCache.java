package com.github.zhuxiaohao.common_library.service;

import com.github.zhuxiaohao.common_library.controller.HttpConstants;
import com.github.zhuxiaohao.common_library.dao.HttpCacheDao;
import com.github.zhuxiaohao.common_library.dao.impl.HttpCacheDaoImpl;
...
import com.github.zhuxiaohao.common_library.util.ArrayUtils;
import com.github.zhuxiaohao.common_library.util.HttpUtils;
...
public class HttpCache {
    ...
    public static final Executor THREAD_POOL_EXECUTOR = java.util.concurrent.Executors.newFixedThreadPool(SystemUtils.DEFAULT_THREAD_POOL_SIZE);
     ...
     */
    public void httpGet(String url, HttpCache.HttpCacheListener listener) {
        // if bigger than android 4.0 use executeOnExecutor, else use execute
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB) {
            new HttpCache.HttpCacheStringAsyncTask(listener).executeOnExecutor(THREAD_POOL_EXECUTOR, url);
        } else {
            new HttpCache.HttpCacheStringAsyncTask(listener).execute(url);
        }
    }
    ...
    private class HttpCacheStringAsyncTask extends android.os.AsyncTask<String, Void, HttpResponse> {
        ...
        private HttpCache.HttpCacheListener listener;

        public HttpCacheStringAsyncTask(HttpCache.HttpCacheListener listener) {
    ...
    private class HttpCacheRequestAsyncTask extends android.os.AsyncTask<HttpRequest, Void, HttpResponse> {
        ...
        public HttpCacheRequestAsyncTask(HttpCache.HttpCacheListener listener) {
            this.listener = listener;
        }
